...
#
# Changelog:
# v0.4.0: Major Feature Update.
#         - Added persistence via params.txt, settings now load on startup and save on generation.
#         - Added "Apply Parameters" (↙️) button to parse infotext from prompt box or load from params.txt.
#         - Added "Save/Load Preset" buttons to manage multiple configurations via .json files.
# v0.3.1: Bug fix. Explicitly set do_not_save_samples=True to prevent TypeError on image save.
# v0.3.0: Feature. Added "Send to inputs" functionality.
#
...
def parse_infotext(infotext):
    ...
    # Negative prompt is special since it can be multi-line
    ...
    # Other single-line parameters
    patterns = {
        'steps': r"Steps: (\d+)",
        'sampler_name': r"Sampler: ([\w\+\s]+),?",
...
def create_infotext(params):
    ...
    # Format the string
    infotext = f"{p['prompt']}\n"
    infotext += f"Negative prompt: {p['negative_prompt']}\n"
...
def create_streamer_ui():
    ...
    # --- Part 1: Load Persistent Settings on Startup ---
    default_params = {
    ...
    with gr.Blocks() as streamer_tab:
        ...
        def on_gallery_select(evt: gr.SelectData):
            ...
            return {
                prompts_input: updated_prompts, negative_prompt: params["negative_prompt"], steps: params["steps"],
        ...
        def apply_parameters(prompts_text):
            """Part 2: Dual-function logic for the ↙️ button."""
            text_to_parse = ""
            if prompts_text.strip():
                text_to_parse = prompts_text
            elif os.path.exists(params_file_path):
                with open(params_file_path, 'r', encoding='utf-8') as f:
        ...
        with gr.Row():
            with gr.Column(scale=2):
                ...
                # Hidden component to handle file downloads
        ...
        stream_button.click(
            fn=process_and_stream_images, inputs=all_ui_components, outputs=[output_gallery, stream_button]
        )
        ...
        apply_button.click(fn=apply_parameters, inputs=[prompts_input], outputs=all_ui_components)
        
        output_gallery.select(fn=on_gallery_select, inputs=[], outputs=all_ui_components)
        
        save_preset_button.click(fn=save_preset, inputs=all_ui_components, outputs=[save_file_output])
        load_preset_button.upload(fn=load_preset, inputs=[load_preset_button], outputs=all_ui_components)
